...
module RubyLint
  ...
  class Options
    E_STDIN_NO_PATH = '-s/--stdin requires exactly one path, relative to the ' \
      'root of the project. RubyLint will use this path to determine which ' \
    ...
    def add_severity_option(opts)
      table = RubyLint::Cop::Severity::CODE_TABLE.merge(A: :autocorrect)
      option(opts, '--fail-level SEVERITY',
             RubyLint::Cop::Severity::NAMES + [:autocorrect],
             table) do |severity|
  ...
  module OptionsHelp
    MAX_EXCL = RubyLint::Options::DEFAULT_MAXIMUM_EXCLUSION_ITEMS.to_s
    FORMATTER_OPTION_LIST = RubyLint::Formatter::FormatterSet::BUILTIN_FORMATTERS_FOR_KEYS.keys
    ...
    TEXT = {
      ...
      list_target_files:                'List all files RubyLint will inspect.',
