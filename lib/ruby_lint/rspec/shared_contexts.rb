...
RSpec.shared_context 'isolated environment', :isolated_environment do
  around do |example|
    Dir.mktmpdir do |tmpdir|
      ...
      RubyLint::FileFinder.root_level = tmpdir
      ...
      begin
        ...
        RubyLint::PathUtil.chdir(working_dir) do
          example.run
        end
      ensure
        ...
        RubyLint::FileFinder.root_level = nil
      end
    end
  end
end
...
RSpec.shared_context 'config', :config do # rubocop:disable Metrics/BlockLength
  ...
  let(:cop_class) do
    if described_class.is_a?(Class) && described_class < RubyLint::Cop::Cop
      described_class
    else
      RubyLint::Cop::Cop
    end
  ...
  let(:config) do
    ...
    RubyLint::Config.new(hash, "#{Dir.pwd}/.rubocop.yml")
  end
