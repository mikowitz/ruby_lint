...
RSpec.describe RubyLint::Cop::Generator do
  ...
  describe '#write_source' do
    ...
    it 'generates a helpful source file with the name filled in' do
      ...
      expect(File)
        .to receive(:write)
        .with('lib/ruby_lint/cop/style/fake_cop.rb', generated_source)

      generator.write_source

      expect(stdout.string)
        .to eq("[create] lib/ruby_lint/cop/style/fake_cop.rb\n")
    end
    ...
    it 'refuses to overwrite existing files' do
      ...
      expect { new_cop.write_source }
        .to output(
          'rake new_cop: lib/ruby_lint/cop/layout/indentation_style.rb '\
          "already exists!\n"
        ).to_stderr
    end
  end
  ...
  describe '#write_spec' do
    ...
    it 'generates a helpful starting spec file with the class filled in' do
      generated_source = <<~SPEC
        ...
        RSpec.describe RubyLint::Cop::Style::FakeCop do
      ...
      expect(File)
        .to receive(:write)
        .with('spec/ruby_lint/cop/style/fake_cop_spec.rb', generated_source)
    ...
    it 'refuses to overwrite existing files' do
      ...
      expect { new_cop.write_spec }
        .to output(
          'rake new_cop: spec/ruby_lint/cop/layout/indentation_style_spec.rb '\
          "already exists!\n"
        ).to_stderr
    end
  end
  ...
  describe '#inject_config' do
    ...
    before do
      ...
      stub_const('RubyLint::Version::STRING', '0.58.2')
    end
  ...
  describe 'compliance with rubocop', :isolated_environment do
    ...
    around do |example|
      new_global = RubyLint::Cop::Registry.new(
        [RubyLint::Cop::InternalAffairs::NodeDestructuring]
      )
      RubyLint::Cop::Registry.with_temporary_global(new_global) { example.run }
    end

    let(:config) do
      config = RubyLint::ConfigStore.new
      path = File.join(RubyLint::ConfigLoader::RUBY_LINT_HOME,
                       RubyLint::ConfigLoader::DOTFILE)
      config.options_config = path
      config
    end
    let(:options) { { formatters: [] } }
    let(:runner) { RubyLint::Runner.new(options, config) }
