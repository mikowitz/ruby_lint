...
RSpec.describe RubyLint::Cop::Team do
  subject(:team) { described_class.mobilize(cop_classes, config, options) }

  let(:cop_classes) { RubyLint::Cop::Cop.registry }
  let(:config) { RubyLint::ConfigLoader.default_configuration }
  let(:options) { nil }
  let(:ruby_version) { RubyLint::TargetRuby.supported_versions.last }
  ...
  context 'when incompatible cops are correcting together' do
    ...
    let(:runner) { RubyLint::Runner.new(options, RubyLint::ConfigStore.new) }
  ...
  describe '#inspect_file', :isolated_environment do
    ...
    let(:offenses) do
      source = RubyLint::ProcessedSource.from_file(file_path, ruby_version)
    ...
    it 'returns offenses' do
      ...
      expect(offenses).to all(be_a(RubyLint::Cop::Offense))
    end
    ...
    context 'when Parser reports non-fatal warning for the file' do
      ...
      context 'when a cop has no interest in the file' do
        it 'returns all offenses except the ones of the cop' do
          allow_any_instance_of(RubyLint::Cop::Layout::LineLength)
    ...
    context 'when Cop#on_* raises an error' do
      ...
      before do
        allow_any_instance_of(RubyLint::Cop::Style::NumericLiterals)
  ...
  describe '#cops' do
    ...
    it 'returns cop instances' do
      ...
      expect(cops.all? { |c| c.is_a?(RubyLint::Cop::Cop) }).to be_truthy
    end

    context 'when only some cop classes are passed to .new' do
      let(:cop_classes) do
        RubyLint::Cop::Registry.new(
          [RubyLint::Cop::Lint::Void, RubyLint::Cop::Layout::LineLength]
        )
      end
    ...
    context 'when some classes are disabled with config' do
      let(:disabled_config) do
        %w[
          Lint/Void
          Layout/LineLength
        ].each_with_object(RubyLint::Config.new) do |cop_name, accum|
          accum[cop_name] = { 'Enabled' => false }
        end
      end
      let(:config) do
        RubyLint::ConfigLoader.merge_with_default(disabled_config, '')
      end
  ...
  describe '#forces' do
    ...
    it 'returns force instances' do
      ...
      forces.each do |force|
        expect(force.is_a?(RubyLint::Cop::Force)).to be(true)
      end
    end

    context 'when a cop joined a force' do
      let(:cop_classes) do
        RubyLint::Cop::Registry.new([RubyLint::Cop::Lint::UselessAssignment])
      end

      it 'returns the force' do
        expect(forces.size).to eq(1)
        expect(forces.first.is_a?(RubyLint::Cop::VariableForce)).to be(true)
      end
    end
    ...
    context 'when multiple cops joined a same force' do
      let(:cop_classes) do
        RubyLint::Cop::Registry.new(
          [
            RubyLint::Cop::Lint::UselessAssignment,
            RubyLint::Cop::Lint::ShadowingOuterLocalVariable
          ]
        )
      end
    ...
    context 'when no cops joined force' do
      let(:cop_classes) do
        RubyLint::Cop::Registry.new([RubyLint::Cop::Style::For])
      end
  ...
  describe '#external_dependency_checksum' do
    let(:cop_classes) { RubyLint::Cop::Registry.new }
  ...
  describe '.new' do
    ...
    it 'accepts cops directly classes' do
      cop = RubyLint::Cop::Metrics::AbcSize.new
