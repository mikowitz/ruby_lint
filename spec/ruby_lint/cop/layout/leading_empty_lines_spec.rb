...
RSpec.describe RubyLint::Cop::Layout::LeadingEmptyLines, :config do
  ...
  context 'auto-correct' do
    context 'in collaboration' do
      let(:config) do
        RubyLint::Config.new('Layout/SpaceAroundEqualsInParameterDefault' => {
      ...
      let(:cops) do
        cop_classes = [
          described_class,
          ::RubyLint::Cop::Layout::SpaceAroundEqualsInParameterDefault
        ]
        ::RubyLint::Cop::Registry.new(cop_classes)
      end
      ...
      it 'does not invoke conflicts with other cops' do
        ...
        team = RubyLint::Cop::Team.mobilize(cops, config, options)
