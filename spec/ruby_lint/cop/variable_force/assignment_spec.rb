...
RSpec.describe RubyLint::Cop::VariableForce::Assignment do
  include RuboCop::AST::Sexp

  let(:ast) do
    RubyLint::ProcessedSource.new(source, ruby_version).ast
  end
  ...
  let(:scope) { RubyLint::Cop::VariableForce::Scope.new(def_node) }
  let(:variable) do
    RubyLint::Cop::VariableForce::Variable.new(name, lvasgn_node, scope)
  end
  let(:assignment) { described_class.new(lvasgn_node, variable) }

  describe '.new' do
    let(:variable) do
      instance_double(RubyLint::Cop::VariableForce::Variable)
    end
