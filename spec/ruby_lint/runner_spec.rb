...
RSpec.describe RubyLint::Runner, :isolated_environment do
  include FileHelper
  ...
  describe '#run when interrupted' do
    ...
    let(:runner) { described_class.new({}, RubyLint::ConfigStore.new) }
  ...
  describe '#run' do
    subject(:runner) { described_class.new(options, RubyLint::ConfigStore.new) }
    ...
    context 'if a cop crashes' do
      before do
        ...
        cache = instance_double(RubyLint::ResultCache, 'valid?' => false)
        # ... but there's a crash in one cop.
        runner.errors = ['An error occurred in ...']

        allow(RubyLint::ResultCache).to receive(:new) { cache }
      end
  ...
  describe '#run with cops autocorrecting each-other' do
    subject(:runner) do
      runner_class = Class.new(RubyLint::Runner) do
        def mobilized_cop_classes(_config)
          RubyLint::Cop::Registry.new(
            [
              RubyLint::Cop::Test::ClassMustBeAModuleCop,
              RubyLint::Cop::Test::ModuleMustBeAClassCop
            ]
          )
        end
      end
      runner_class.new(options, RubyLint::ConfigStore.new)
    end
    ...
    context 'if there is an offense in an inspected file' do
      ...
      it 'aborts because of an infinite loop' do
        expect do
          runner.run([])
        end.to raise_error RubyLint::Runner::InfiniteCorrectionLoop
      end
    end
  end
end
