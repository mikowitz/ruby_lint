...
RSpec.describe RubyLint::Formatter::DisabledConfigFormatter, :isolated_environment do
  include FileHelper
  ...
  let(:offenses) do
    [RubyLint::Cop::Offense.new(:convention, location, 'message', 'Cop1'),
     RubyLint::Cop::Offense.new(:convention, location, 'message', 'Cop2')]
  end
  ...
  context "when there's .rubocop.yml" do
    before do
      ...
      allow(RubyLint::Cop::Cop.registry).to receive(:contains_cop_matching?)
        .and_return(true)
      formatter.finished(['test_a.rb', 'test_b.rb'])
    end
