...
RSpec.describe RubyLint::Formatter::HTMLFormatter, :isolated_environment do
  ...
  around do |example|
    ...
    RubyLint::PathUtil.chdir(File.basename(project_path)) do
      example.run
    end
  end
  ...
  let(:actual_html_path) do
    ...
    # different platforms.
    RubyLint::CLI.new.run(['--except', 'Layout/EndOfLine', '--format', 'html',
                          '--out', path])
    path
  end
  ...
  let(:expected_html) do
    ...
    html.sub(/(class="version".{0,20})\d+(?:\.\d+){2}/i) do
      Regexp.last_match(1) + RubyLint::Version::STRING
    end
  end
