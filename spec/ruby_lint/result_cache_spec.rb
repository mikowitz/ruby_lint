...
RSpec.describe RubyLint::ResultCache, :isolated_environment do
  include FileHelper
  ...
  let(:registry) { RubyLint::Cop::Cop.registry }
  let(:team) do
    RubyLint::Cop::Team.mobilize(
      registry,
      RubyLint::ConfigLoader.default_configuration,
      options
    )
  end
  ...
  let(:options) { {} }
  let(:config_store) do
    instance_double(RubyLint::ConfigStore, for_dir: RubyLint::Config.new)
  end
  let(:cache_root) { "#{Dir.pwd}/rubocop_cache" }
  let(:offenses) do
    [RubyLint::Cop::Offense.new(:warning, location, 'unused var',
  ...
  before do
    ...
    allow(config_store).to receive(:for_file).with('example.rb')
                                             .and_return(RubyLint::Config.new)
  ...
  describe 'cached result that was saved with no command line option' do
    ...
    context 'when no option is given' do
      ...
      context 'when file permission have changed' do
        unless RubyLint::Platform.windows?
          it 'is invalid' do
            cache.save(offenses)
      ...
      context 'when a symlink is present in the cache location' do
        ...
        before do
          ...
          # machine" on Windows.
          skip 'Symlinks not implemented on Windows' if RubyLint::Platform.windows?
    ...
    context 'when the default internal encoding is UTF-8' do
      let(:offenses) do
        [
          ...
          RubyLint::Cop::Offense.new(:warning, location, message,
                                    'Lint/UselessAssignment')
        end
      end
  ...
  describe '.cleanup' do
    before do
      cfg = RubyLint::Config.new('AllCops' => { 'MaxFilesInCache' => 1 })
  ...
  describe 'the cache path' do
    let(:config_store) do
      instance_double(RubyLint::ConfigStore)
    end
    ...
    before do
      ...
      config = RubyLint::Config.new(all_cops)
