...
RSpec.describe RubyLint::CLI, :isolated_environment do
  include_context 'cli spec behavior'

  subject(:cli) { described_class.new }

  let(:rubocop) { "#{RubyLint::ConfigLoader::RUBY_LINT_HOME}/exe/ruby_lint" }
  ...
  describe '--version' do
    it 'exits cleanly' do
      ...
      expect($stdout.string).to eq((RubyLint::Version::STRING + "\n") * 2)
    end
  end
  ...
  describe '-V' do
    it 'exits cleanly' do
      expect(cli.run(['-V'])).to eq(0)
      expect($stdout.string).to include(RubyLint::Version::STRING)
  ...
  describe '--only' do
    context 'when one cop is given' do
      ...
      it 'accepts cop names from plugins' do
        ...
        rubocop = "#{RubyLint::ConfigLoader::RUBY_LINT_HOME}/exe/ruby_lint"
  ...
  describe '--show-cops' do
    ...
    let(:global_conf) do
      config_path =
        RubyLint::ConfigLoader.configuration_file_for(Dir.pwd.to_s)
      RubyLint::ConfigLoader.configuration_from_file(config_path)
    end
      ...
      context 'when a class name is specified' do
        it 'uses the class as a formatter' do
          stub_const('MyTool::RuboCopFormatter',
                     Class.new(RubyLint::Formatter::BaseFormatter) do
