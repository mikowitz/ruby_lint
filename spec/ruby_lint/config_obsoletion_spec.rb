...
RSpec.describe RubyLint::ConfigObsoletion do
  include FileHelper

  subject(:config_obsoletion) { described_class.new(configuration) }

  let(:configuration) { RubyLint::Config.new(hash, loaded_path) }
    ...
    context 'when the configuration includes any obsolete cop name' do
      ...
      it 'prints a warning message' do
        begin
          ...
          raise 'Expected a RubyLint::ValidationError'
        rescue RubyLint::ValidationError => e
