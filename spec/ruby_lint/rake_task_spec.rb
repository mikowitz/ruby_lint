...
require 'ruby_lint/rake_task'

RSpec.describe RubyLint::RakeTask do
  include FileHelper
  ...
  describe 'running tasks' do
    ...
    it 'runs with default options' do
      ...
      cli = instance_double(RubyLint::CLI, run: 0)
      allow(RubyLint::CLI).to receive(:new).and_return(cli)
    ...
    it 'will not error when result is not 0 and fail_on_error is false' do
      ...
      cli = instance_double(RubyLint::CLI, run: 1)
      allow(RubyLint::CLI).to receive(:new).and_return(cli)
